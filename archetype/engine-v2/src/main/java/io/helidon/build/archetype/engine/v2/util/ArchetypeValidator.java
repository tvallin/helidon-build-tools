...
public final class ArchetypeValidator implements Node.Visitor<Context>, Block.Visitor<Context> {
    ...
    public VisitResult visitPreset(Preset preset, Context ctx) {
        ctx.scope().putValue(preset.path(), preset.value(), ValueKind.LOCAL_VAR, preset.isModel());
        presets.add(preset);
        return VisitResult.CONTINUE;
    }
    ...
    public VisitResult visitVariable(Variable variable, Context ctx) {
        ContextValue value = ctx.putValue(variable.path(), variable.value(), ValueKind.LOCAL_VAR, variable.isModel());
